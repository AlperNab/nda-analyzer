...
**Domain:** `Legal / NDA Review` · **Suite:** `Legal & Compliance Suite` · **Accent:** `#c084fc`
...
**Nda Analyzer** is a standalone, browser-based AI workflow app for **Legal / NDA Review**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.
...
|---|---:|:---:|---|---|
| `nda`<br>NDA | text | Yes | — | Affects input: NDA. |
...
| `party_role`<br>party role | select | No | buyer/client | Affects customization: party role. |
| `jurisdiction`<br>jurisdiction | select | No | United States | Affects customization: jurisdiction. |
| `company_size`<br>company size | text | No | — | Affects customization: company size. |
| `risk_tolerance`<br>risk tolerance | slider | No | 50 | Affects customization: risk tolerance. |
| `negotiation_style`<br>negotiation style | select | No | — | Affects customization: negotiation style. |
| `fallback_clause_library`<br>fallback clause library | text | No | — | Affects customization: fallback clause library. |
...
uvicorn app.main:app --host 127.0.0.1 --port 9139
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9139`. |
...
```bash
curl http://127.0.0.1:9139/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9139/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9139 uvicorn app.main:app --host 0.0.0.0 --port 9139
```
...
- `plugins/nda-analyzer.json` for plugin metadata.
